...
public class Table {
	...
	public void update(String[] columns, String[] values, Condition where) throws SQLException {
		...
		for (Row row : rows) {
			//if each row satisfies the where clause
			if (row.satisfies(where, description))
			{
				for(int i=0; i<columns.length; i++)
				{
					//if it does then update
					row.data.replace(columns[i], values[i]);
				}
			}
		}
	}
	...
	public void delete(Condition where) throws SQLException {
		...
		Field[] columns = description.fields();
		
		//declare removal and current of Position
		Position<Row> removal = null;
		...
		for(Row row : rows)
		{
			...
			if(row.satisfies(where, description))
			{
				...
				{
					//remove row
					row.data.remove(columns[i].name());
					removal = current;
				}
			}
			//next row is now current
