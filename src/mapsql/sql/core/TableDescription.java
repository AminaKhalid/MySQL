...
public class TableDescription {
	...
	public void checkForNotNulls(String[] cols) throws SQLException {
		...
		{
			//create boolean to check if NOT NULL field is true
			if(field.isNotNull())
			{
				boolean checked = false;
				//iterate through columns to find field
				for(String column : cols)
				{
					if(column.equals(field.name()))
					{
						checked = true;
					}
				}
				if(!checked)
				{
					//throw exception if a field is declared as NOT NULL
